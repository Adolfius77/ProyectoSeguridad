...
class LogicaChatTCP:
    ...
    def __init__(self, usuarioDTO: UsuarioDTO, emisor: IEmisor, modelo_chat_tcp=None):
        """
    ...
    def enviarMensajePrivado(self, usuarioDestino: UsuariosOP, contenidoMensaje: str) -> None:
        """
        Crea un MensajeDTO y lo envía como PaqueteDTO a un usuario específico.
    ...
    def enviarMensajeGrupal(self, contenidoMensaje: str) -> None:
        """
        Envía un mensaje grupal (broadcast) a todos los usuarios conectados.
        ...
        # Crear paquete con tipo de evento ENVIAR_MENSAJE y destino TODOS
        paqueteDTO = PaqueteDTO(
            tipo=TiposEvento.mensajeGrupal,
            contenido=mensajeDTO.to_dict(),
            origen=self.usuarioEnSesion.nombre_usuario,
    ...
    def registrarseEnEventBus(self, host_bus: str, puerto_bus: int) -> None:
        ...
        """
        eventos = [
            TiposEvento.unirseGrupal,
            TiposEvento.mensajePrivado,
            TiposEvento.mensajeGrupal
        ]

        self._logger.info(f"Registrándose en EventBus para eventos: {eventos}")

        paqueteDTO = PaqueteDTO(
            tipo=TiposEvento.iniciarConexion,
    ...
    def procesarEvento(self, paquete: PaqueteDTO) -> None:
        ...
        try:
            ...
            if tipo == "MENSAJE_PRIVADO":
                self._procesarMensajePrivado(paquete)

            elif tipo == "MENSAJE_GRUPAL":
