...
class ReceptorLogicaServidor(IReceptor):
    ...
    def clliente_tcp(self):
        return self.event_bus.clliente_tcp
    ...
    def _procesar_login(self, paquete):
        ...
        if repositorioUsuarios.validar(user, datos['password']):
            ...
            if len(conectados) >=5:
                self._enviar_respuesta_directa(paquete.host, datos['puerto_escucha'], datos['public_key'], "ERROR","Servidor Lleno")
                return
            llave_publica_user = datos['public_key'].encode('utf-8')
            ...
            self.event_bus.registrar_servicio("MEMSAJE", nuevo_servicio)
    ...
    def _procesar_mensaje(self, paquete):
        ...
        if destino == "TODOS":
            ...
            for servicio in subcriptores:
                self._enviar_paquete_seguro(servicio, "MENSAJE", paquete.contenido, origen=paquete.origen, destino=destino)
